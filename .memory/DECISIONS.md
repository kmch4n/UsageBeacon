...
## D-015: Compact old usage into exact lifetime token totals

- Date: 2026-07-29
- Status: Active
- Decision: Keep detailed `TokenUsageEntry` values for 180 days, then compact older entries into input/output token totals while retaining every archived `IdHash` for exact deduplication. Show this as locally retained usage rather than an account lifetime total.
- Reason: D-011 deliberately bounded the cache when only 30-day views existed. The dashboard now exposes a lifetime token total, so dropping old values would make that number decrease. Keeping every full entry forever would preserve correctness but make every JSON load and save grow with all historical model and timestamp details. Token-only compaction retains the requested information while keeping detailed storage bounded.
- Consequences: Compaction uses deterministic case-insensitive path order and archives each identity once. A later parser migration or modified file may reintroduce an old entry, but its archived identity prevents double counting. Archived totals use `decimal` so accumulating valid `long` counters cannot wrap negative. The archive still grows by one 64-bit identity per unique old usage event; SQLite or another incremental store should be reconsidered if measured load/save latency becomes material. Cache writes occur only after a change and serialize directly to a temporary stream before replacement. The UI says "Recorded on this PC" and discloses that deleted-before-scan logs, prior cache loss, unreadable files, and unrecoverable pre-migration history can leave gaps. Parser revisions force a one-time reparse of still-present Claude and Codex logs to recover previously dropped entries where possible.
