...
The 2026-09-12 amendment for #18 covers recognized credential keys in quoted structured fields as well as unquoted assignments. Quoted values are consumed completely, including whitespace and escaped quotes. Unterminated strings, compound values, and escaped representations conservatively omit the remaining record rather than guessing where the value ends. Existing unquoted values retain punctuation-inclusive masking. Redaction remains best effort over arbitrary exception text, not a guarantee that a log is safe to publish; attachment guidance requires reviewing the complete record. No telemetry or additional logging mode is introduced.
...
## D-015: Retain pricing-neutral usage for lifetime cost estimates

- Date: 2026-07-29
- Amended: 2026-07-30
- Status: Active
- Decision: Keep per-file `TokenUsageEntry` values for 180 days, then move older events into a path-independent archive that retains the exact timestamp, service, model, five token buckets, and `IdHash`. Recalculate the locally retained lifetime Claude and Codex USD estimates from this pricing-neutral history whenever the dashboard loads.
- Reason: D-011 deliberately bounded the cache when only 30-day views existed. A lifetime cost split cannot be reconstructed from token-only totals because service, model, price-effective timestamp, and cache-token categories all affect the result. Persisting calculated dollars would also freeze old results when the pricing catalog is corrected. Exact usage fields are therefore the smallest representation that preserves historical repricing, arbitrary effective-time boundaries, future-record exclusion, and service separation.
- Consequences: Archiving uses deterministic case-insensitive path order and retains each identity once. A later parser migration or modified file may reintroduce an old event, but its archived identity prevents double counting. Schema v2 reparses schema-v1 files once to recover token-only archive identities into exact events; totals whose source logs no longer exist remain explicitly unpriced, and the UI shows an explicit coverage note instead of inventing an allocation. Repeated archived model names are table-encoded and event fields use compact positional JSON rows. The archive still grows linearly with unique old usage events, so SQLite should be reconsidered if measured load/save latency becomes material. Cache writes occur only after a change and serialize directly to a temporary stream before replacement. The UI says "Recorded on this PC" and discloses that deleted-before-scan logs, prior cache loss, unreadable files, and unrecoverable pre-migration history can leave gaps.
- Evidence: [`UsageBeacon/Services/Insights/UsageLogCache.cs`](../UsageBeacon/Services/Insights/UsageLogCache.cs), [`UsageBeacon/Services/Insights/UsageAggregator.cs`](../UsageBeacon/Services/Insights/UsageAggregator.cs), [`UsageBeacon/Views/DashboardWindow.xaml`](../UsageBeacon/Views/DashboardWindow.xaml), and [`docs/DASHBOARD.md`](../docs/DASHBOARD.md).

## D-016: Bound complete Claude HTTP responses

- Date: 2026-09-12
- Status: Active
- Decision: Both Claude HTTP clients use `ResponseContentRead` so HttpClient enforces its existing timeout and 64 KiB buffer limit through the end of the response body. Defaults remain 10 seconds for usage and 30 seconds for OAuth refresh.
